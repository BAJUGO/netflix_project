...
import project_dir.views_part.global_crud as crud
...
async def delete_author(author_id: int, session: AsyncSession = ses_dep):
    return await global_crud.delete_author_session(session, author_id)
...
async def full_update_author(author_id: int, author_schema: schemas.AuthorCreate,
    ...
    return await global_crud.full_update_author_session(session, author_id, author_schema)
...
async def patch_update_author(author_id: int, author_schema: schemas.AuthorPatch,
                              session: AsyncSession = ses_dep):
    return await global_crud.patch_author_session(session, author_id, author_schema)
...
async def delete_movie(movie_id: int, session: AsyncSession = ses_dep):
    return await global_crud.delete_movie_session(session, movie_id)
...
async def full_update_movie(movie_id: int, movie_schema: schemas.MovieCreate,
                            session: AsyncSession = ses_dep):
    return await global_crud.full_update_movie_session(session, movie_id, movie_schema)
...
async def patch_update_movie(movie_id: int, movie_schema: schemas.MoviePatch,
                             session: AsyncSession = ses_dep):
    return await global_crud.patch_movie_session(session, movie_id, movie_schema)
...
async def delete_series(series_id: int, session: AsyncSession = ses_dep):
    return await global_crud.delete_series_session(session, series_id)
...
async def full_update_series(series_id: int, series_schema: schemas.SeriesCreate,
                             session: AsyncSession = ses_dep):
    return await global_crud.full_update_series_session(session, series_id, series_schema)
...
async def patch_update_series(series_id: int, series_schema: schemas.SeriesPatch,
                              session: AsyncSession = ses_dep):
    return await global_crud.patch_series_session(session, series_id, series_schema)
...
async def delete_user(user_id: int, session: AsyncSession = ses_dep):
    return await global_crud.delete_user_session(user_to_delete_id=user_id, session=session)
...
async def change_user_role(user_id: int, role_to_change: str = Form(...),
                           session: AsyncSession = ses_dep):
    return await global_crud.change_role_session(session, user_id, role_to_change)
