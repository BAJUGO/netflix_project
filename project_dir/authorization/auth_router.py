...
from project_dir.core import db_helper, ses_dep
...
async def delete_user(user_id: int, session: AsyncSession = ses_dep, admin=admin_dep):
...
async def change_user_role(user_id: int, role_to_change: str = Form(...),
                           session: AsyncSession = ses_dep, admin=admin_dep):
