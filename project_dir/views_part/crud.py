...
async def adder_session(data: BaseModel, session: AsyncSession, orm_model: type[T]) -> T:
...
async def deleter_session(session: AsyncSession, obj_id: int, orm_model: type[T]) -> None:
    obj_to_del = await getter_by_id_session(session=session, orm_model=orm_model, obj_id=obj_id)
    await session.delete(obj_to_del)
    await session.commit()


async def patch_updater_session(session: AsyncSession, obj_id: int, orm_model: type[T], schema_patch: P) -> T:
    data_to_update = schema_patch.model_dump(exclude_unset=True)
    await session.execute(update(orm_model).where(orm_model.id == obj_id).values(**data_to_update))
    await session.commit()
    return await getter_by_id_session(session, orm_model, obj_id)


async def full_updater_session(session: AsyncSession, obj_id: int, orm_model: type[T], schema_patch: P) -> T:
...
# ! PATCH
async def patch_author_session(session: AsyncSession, author_id: int, author_schema: AuthorPatch) -> Author:
    return await patch_updater_session(session, author_id, Author, author_schema)


async def patch_movie_session(session: AsyncSession, movie_id: int, movie_schema: MoviePatch) -> Movie:
    return await patch_updater_session(session, movie_id, Movie, movie_schema)


async def patch_series_session(session: AsyncSession, series_id: int, series_schema: SeriesPatch) -> Series:
...
async def full_update_movie_session(session: AsyncSession, movie_id: int, movie_schema: MovieCreate) -> Movie:
    return await full_updater_session(session, movie_id, Movie, movie_schema)


async def full_update_series_session(session: AsyncSession, series_id: int, series_schema: SeriesCreate) -> Series:
